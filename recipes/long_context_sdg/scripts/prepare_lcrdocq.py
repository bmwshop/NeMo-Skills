...
prompt_template_documents = """BEGIN INPUT DOCUMENTS
...
def construct_docs_prompt(docs, prompt_template=prompt_template_documents):
    documents_text = "\n\n".join(f"BEGIN DOCUMENT {i + 1}:\n{doc}\nEND DOCUMENT {i + 1}" for i, doc in enumerate(docs))
...
def write_data_to_file(output_file, data, txt_file_folder, max_context_window, tokenizer_name):
    with open(output_file, "wt", encoding="utf-8") as fout:
        for idx, entry in tqdm(enumerate(data), desc=f"Writing to {output_file}"):
            ...
            docs = construct_docs_prompt(documents)
...
def prepare_aalcr_data(max_context_window, setup, tokenizer_name):
    ...
    output_file = "/lustre/fsw/portfolios/llmservice/users/drekesh/DATA/lc/lcrdocq_topic_annotation/lcrdocq.jsonl"
