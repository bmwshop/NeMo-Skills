...
if category == "sec":
    input_file = f"/workspace/DATA/lc/lcrqagen_v3b/sec/{template}-{args.data_chunk_size}-{args.data_maxlength}/intermediate_p.jsonl"
    output_dir = f"/workspace/DATA/lc/lcrqagen_v3b/sec/{template}-{args.data_chunk_size}-{args.data_maxlength}/critiques"
else:
    input_file = f"/workspace/DATA/lc/lcrqagen_v3b/{template}-{args.data_chunk_size}-{args.data_maxlength}/intermediate_p.jsonl"
    output_dir = f"/workspace/DATA/lc/lcrqagen_v3b/{template}-{args.data_chunk_size}-{args.data_maxlength}/critiques"


# prompt_config = "/nemo_run/code/recipes/long_context_sdg/prompts/gen_trace.yaml"
prompt_config = "/nemo_run/code/recipes/long_context_sdg/prompts/v3b/critique_nothink.yaml"
...
generate(
    ...
    postprocess_cmd=f"python /nemo_run/code/recipes/long_context_sdg/scripts/postprocess_qa.py --input {output_dir}/output.jsonl --output {output_dir}/critique.jsonl",
    server_gpus=4,
    server_nodes=2,
)
