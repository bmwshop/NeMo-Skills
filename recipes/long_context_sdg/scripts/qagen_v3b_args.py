...
if category == "sec":
    ...
    output_dir = f"/workspace/DATA/lc/lcrqagen_v3b/sec/{template}-{args.data_chunk_size}-{args.data_maxlength}"
else:
    ...
    output_dir = f"/workspace/DATA/lc/lcrqagen_v3b/{template}-{args.data_chunk_size}-{args.data_maxlength}"
...
prompt_config = f"/nemo_run/code/recipes/long_context_sdg/prompts/v3b/{template}.yaml"
...
generate(
    ...
    postprocess_cmd=f"python /nemo_run/code/recipes/long_context_sdg/scripts/postprocess_v3b.py --input {output_dir}/output.jsonl --output {output_dir}/qa.jsonl",
    # Server parameters
