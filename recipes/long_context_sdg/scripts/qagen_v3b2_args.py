...
if category == "sec":
    ...
    input_file = f"/workspace/DATA/lcr_docs/simple/sec/sec-{args.data_chunk_size}-{args.data_maxlength}.simple.sh.v3b2.jsonl"
    output_dir = f"/workspace/DATA/lc/lcrqagen_v3b2/sec/{template}-{args.data_chunk_size}-{args.data_maxlength}"
else:
    # input_file = f"/workspace/DATA/lcr_docs/acgilms-{args.data_chunk_size}-{args.data_maxlength}.jsonl"
    input_file = f"/workspace/DATA/lcr_docs/simple/acgilms-{args.data_chunk_size}-{args.data_maxlength}_sh_v3b2.jsonl"
    output_dir = f"/workspace/DATA/lc/lcrqagen_v3b2/{template}-{args.data_chunk_size}-{args.data_maxlength}"
...
prompt_config = f"/nemo_run/code/recipes/long_context_sdg/prompts/v3b2/{template}.yaml"
...
generate(
    ...
    postprocess_cmd=f"python /nemo_run/code/recipes/long_context_sdg/scripts/postprocess_v3b1.py --input {output_dir}/output.jsonl --output {output_dir}/qa.jsonl",
    # Server parameters
